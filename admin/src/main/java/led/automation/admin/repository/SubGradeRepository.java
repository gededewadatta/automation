...
public interface SubGradeRepository extends JpaRepository<SubGrade, Long> {
	...
	@Query(value = "SELECT * FROM SubGrade WHERE Sub_Grade_CODE like %?1", nativeQuery = true)
	SubGrade findBySubGradeCode(String SubGradeCode);
	@Query(value = "SELECT * FROM SubGrade WHERE Sub_Grade_CODE like %?1 AND SubGrade_NAME like %?2", nativeQuery = true)
