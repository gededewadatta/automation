...
//            mLoginFormView.setVisibility(show ? View.GONE : View.VISIBLE);
//        }
       //put to table employee: start
        String email = mEmailView.getText().toString();
        Map<String, Object> map = new HashMap<>();
        map.put("userName", email);

        mFirestore.collection(DEFAULT_COLLECTION)
                .document(DOCUMENT_EMPLOYEE)
                .set(map)
        ...
        //put to table employee: stop
        //get data pending question : start
        PendingQuestion pending = this.customObjects();
        if(pending.getUserName().equals(email)){
            if(pending.getQuestionType().equals("FREETEXT")){
                Intent intent= new Intent(getApplicationContext(),FreeTextQuestionActivity.class);
                intent.putExtra("userName", email);

                startActivity(intent);

            }else if (pending.getQuestionType().equals("OPTIONAL")){
                Intent intent= new Intent(getApplicationContext(),OptionalQuestionActivity.class);
                intent.putExtra("userName", email);
        ...
        //get data pending question : stop
    }
    public PendingQuestion customObjects() {
        // [START custom_objects]
        final PendingQuestion pend = null;
        String inquiry = DOCUMENT_PENDING_QUESTIONS+"."+mEmailView.getText().toString();
       // final PendingQuestion pendingQuestion = null;
